            ...
            namespace mgrams {
                namespace Comp_M_Gram_Id {
                    ...
                    static inline void create_gram_id(const TShortId * word_ids,
                        ...
                        LOG_INFO1 << "M_GRAM_LEVEL: " << (uint32_t) M_GRAM_LEVEL << ", id_type_value: " << id_type_value << END_LOG;
...
#define IS_LARGER +1
                    ...
                    int8_t compare(const T_Id_Storage_Ptr & m_p_gram_id_one, const T_Id_Storage_Ptr & m_p_gram_id_two) {
                        ...
                        LOG_INFO1 << "M_GRAM_LEVEL: " << (uint32_t) M_GRAM_LEVEL << ", type_one: " << type_one << ", type_two: " << type_two << END_LOG;

                        if (type_one < type_two) {
                            //The first id type is smaller
                            LOG_INFO1 << SSTR((void*) m_p_gram_id_one) << " < " << SSTR((void*) m_p_gram_id_two) << END_LOG;
                            return IS_LESS;
                        } else {
                            if (type_one > type_two) {
                                ...
                                LOG_INFO1 << "ID_TYPE_LEN_BITS: " << SSTR((uint32_t) ID_TYPE_LEN_BITS)
