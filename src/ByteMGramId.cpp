    ...
    namespace smt {
        namespace tries {
            namespace mgrams {
                 ...
                 */
                const static uint32_t gram_id_type_mult[] = {1, 4, 4 * 4, 4 * 4 * 4, 4 * 4 * 4 * 4};
                ...
                inline void gram_id_byte_len_2_type(uint8_t len_bytes[M_GRAM_LEVEL], uint32_t & id_type) {
                    ...
                    if (DO_SANITY_CHECKS && (M_GRAM_LEVEL > M_GRAM_LEVEL_5)) {
                        ...
                        msg << "get_gram_id_type: Unsupported m-gram level: "
                                ...
                                << SSTR(M_GRAM_LEVEL_2) << ", "
                                << SSTR(M_GRAM_LEVEL_5) << "], insufficient multipliers!";
                ...
                void get_gram_id_len(const uint8_t * m_gram_id, uint8_t & len_bytes) {
                    ...
                    len_bytes = NUM_BITS_TO_STORE_BYTES(id_len_bytes);
                };
                ...
                static inline void create_2_gram_id(const TShortId * word_ids,
                        T_Gram_Id_Storage_Ptr & m_p_gram_id) {
                    create_gram_id < Byte_M_Gram_Id::M_GRAM_2_TO_5_ID_TYPE_LEN_BYTES, M_GRAM_LEVEL_2 >
                            (word_ids, m_p_gram_id);
                }

                static inline void create_3_gram_id(const TShortId * word_ids,
                        T_Gram_Id_Storage_Ptr & m_p_gram_id) {
                    create_gram_id < Byte_M_Gram_Id::M_GRAM_2_TO_5_ID_TYPE_LEN_BYTES, M_GRAM_LEVEL_3 >
                            (word_ids, m_p_gram_id);
                }

                static inline void create_4_gram_id(const TShortId * word_ids,
                        T_Gram_Id_Storage_Ptr & m_p_gram_id) {
                    create_gram_id < Byte_M_Gram_Id::M_GRAM_2_TO_5_ID_TYPE_LEN_BYTES, M_GRAM_LEVEL_4 >
                            (word_ids, m_p_gram_id);
                }

                static inline void create_5_gram_id(const TShortId * word_ids,
                        T_Gram_Id_Storage_Ptr & m_p_gram_id) {
                    create_gram_id < Byte_M_Gram_Id::M_GRAM_2_TO_5_ID_TYPE_LEN_BYTES, M_GRAM_LEVEL_5 >
                            (word_ids, m_p_gram_id);
                }
                ...
                int Byte_M_Gram_Id::compare(const T_Gram_Id_Storage_Ptr & m_p_gram_id_one, const T_Gram_Id_Storage_Ptr & m_p_gram_id_two) {
                    ...
                    if (DO_SANITY_CHECKS && (M_GRAM_LEVEL > M_GRAM_LEVEL_5)) {
                        ...
                        msg << "get_gram_id_len: Unsupported m-gram level: "
                                << SSTR(M_GRAM_LEVEL) << ", must be within ["
                                << SSTR(M_GRAM_LEVEL_2) << ", "
                                << SSTR(M_GRAM_LEVEL_5) << "], need to set proper type len!";
                        throw Exception(msg.str());
                    }

                    //Get the id len in bits
                    constexpr uint8_t ID_TYPE_LEN_BYTES = Byte_M_Gram_Id::M_GRAM_2_TO_5_ID_TYPE_LEN_BYTES;
                    ...
                    if (type_one < type_two) {
                        ...
                        if (type_one > type_two) {
                            ...
                            get_gram_id_len < ID_TYPE_LEN_BYTES, M_GRAM_LEVEL > (m_p_gram_id_one, id_len_bytes);
