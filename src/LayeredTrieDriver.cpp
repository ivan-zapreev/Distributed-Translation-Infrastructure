...
namespace uva {
    namespace smt {
        namespace tries {
            ...
            template<typename TrieType >
            template<TModelLevel CURR_LEVEL>
            void LayeredTrieDriver<TrieType>::get_prob_weight(const T_M_Gram<WordIndexType> & gram, SQueryResult<MAX_LEVEL> & result) const {
                ...
                if (CURR_LEVEL > M_GRAM_LEVEL_1) {
                    ...
                    if (get_m_gram_ctx_id(gram.template first<false, CURR_LEVEL>(), gram.template last<false>(), ctx_id)) {
                        ...
                        if (CURR_LEVEL == MAX_LEVEL) {
                            ...
                            if (m_trie.template get_m_gram_data_ref<CURR_LEVEL>(word_id, ctx_id, &entry_ptr)) {
                                ...
                                result.m_total_prob = entry_ptr->prob;
                            } else {
                                //Could not compute the probability for
                                //the given level, so backing off (recursive)!
                                LOG_DEBUG << "Unable to find the " << SSTR(CURR_LEVEL)
                    ...
                    if (m_trie.get_1_gram_data_ref(word_id, & entry_ptr)) {
                        ...
                        result.m_total_prob = ZERO_LOG_PROB_WEIGHT;
                    }
                }
            }
            ...
            template<typename TrieType >
            template<TModelLevel CURR_LEVEL>
            void LayeredTrieDriver<TrieType>::add_back_off_weight(const T_M_Gram<WordIndexType> & gram, SQueryResult<MAX_LEVEL> & result) const {
                ...
                if (CURR_LEVEL > M_GRAM_LEVEL_1) {
                    ...
                    if (get_m_gram_ctx_id(gram.template first<true, CURR_LEVEL>(), gram.template last<true>(), ctx_id)) {
                        ...
                        if (m_trie.template get_m_gram_data_ref<CURR_LEVEL>(word_id, ctx_id, &entry_ptr)) {
                            ...
                            result.m_total_prob += entry_ptr->back_off;
                    ...
                    if (m_trie.get_1_gram_data_ref(word_id, &pb_data_ptr)) {
                        ...
                        result.m_total_prob += pb_data_ptr->back_off;
                        LOG_DEBUG << "The 1-Gram log_" << LOG_PROB_WEIGHT_BASE
...
#define INSTANTIATE_LAYERED_DRIVER_TEMPLATES_NAME_TYPE(TRIE_NAME, TYPE) \
            ...
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_1>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_2>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_3>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_4>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_5>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_6>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::get_prob_weight<M_GRAM_LEVEL_7>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_1>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_2>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_3>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_4>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_5>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_6>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
            template void LayeredTrieDriver< T##TRIE_NAME##TYPE >::add_back_off_weight<M_GRAM_LEVEL_7>(const T_M_Gram<T##TRIE_NAME##TYPE::WordIndexType> & gram, SQueryResult<M_GRAM_LEVEL_MAX> & result) const; \
