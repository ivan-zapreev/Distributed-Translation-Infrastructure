            ...
            namespace server {
                namespace lm {
                    ...
                    m_unk_data(NULL), m_mgram_mem_factor(mgram_mem_factor), m_ngram_mem_factor(ngram_mem_factor), m_1_gram_data(NULL) {
                        ...
                        ASSERT_CONDITION_THROW((LM_M_GRAM_LEVEL_MAX < M_GRAM_LEVEL_2),
                                string("The minimum supported trie level is") + std::to_string(M_GRAM_LEVEL_2));
                        ASSERT_CONDITION_THROW((!word_index.is_word_index_continuous()),
                                "This trie can not be used with a discontinuous word index!");
                        ASSERT_CONDITION_THROW((sizeof (uint32_t) != sizeof (word_uid)),
                                string("Only works with a ") + to_string(sizeof (uint32_t)) + string(" byte word_uid!"));
