...
my($src_file,$O_file,@ref_files)=@ARGV;
#my($nbest_file,$src_file,@ref_files)=@ARGV;

my $max_n=4;
my $bp_strategy='closest';

#my $bleu_strategy='liang';
my $bleu_strategy='chiang';
...
my @src_lengths;
&get_src_length_proportions($src_file,\@src_length_proportions,\@src_lengths);

my @O_scores;
if($bleu_strategy eq 'chiang') {
    &read_O_scores($O_file,\@O_scores);
}
...
while(defined(my $line=<STDIN>)) {
    ...
    my $iteration;
    if($line=~/^[0-9]+ \|\|\| iter\-([0-9]+) \|\|\|/) {
	$iteration=$1;
	$line=~s/^([0-9]+ \|\|\|) iter\-[0-9]+ \|\|\|/$1/;
    }

    my($sent_id,$old_bleu_scores,$translation,$scores_string)=split(/ \|\|\| /o,$line);
	...
	print "$sent_id ||| $iteration ||| $bleu $src_length_proportions[$sent_id] $correct_total_ref_lengths_string ||| $translation ||| $scores_string\n";
    } else {
...
sub compute_bleu {
    ...
    } elsif($bleu_strategy eq 'chiang') {
	my $plain_bleu=&compute_sent_bleu($sent_id,$translation,$max_n,$ref_ngrams,$ref_lengths,$bp_strategy,$correct,$total,$ref_length);
	my $prec=0;
...
sub get_src_length_proportions {
    my($src_file,$src_length_proportions,$src_lengths)=@_;
