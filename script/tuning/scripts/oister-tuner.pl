...
my $decoder_wrapper="$OISTERHOME/tuning/scripts/oister-decoder-rescorer-wrapper.pl";
...
sub return_lambda_string_from_ivansConfig
{
    ...
    #@features_specifiers_set{@features_specifiers} = () # set the values of all keys to undef to make the hash to look like a set
    ...
    while(my $line = <CONF_FILE>)
    {
        ...
        if($line !~ /^\#.*$/)
        {
            ...
            #if($line =~ /^lm_feature_weights=(.*)$/)
            #{
            #    push(@lambdas, $1);
            #}
            #elsif($line =~ /^tm_feature_weights=(.*)$/)
            #{
            #    my $tm_feature_str = $1;
            #    my @tm_weights = split(/\|/,$tm_feature_str);
            #    push(@lambdas, @tm_weights);
            #}
            #elsif($line =~ /^rm_feature_weights=(.*)$/)
            #{
            #    my $rm_feature_str =$1;
            #    my @rm_weights = split(/\|/, $rm_feature_str);
            #    push(@lambdas, @rm_weights);
            #}
            #elsif($line =~ /^de_word_penalty=(.*)$/)
            #{
            #    push(@lambdas, $1);
            #}
            #elsif($line =~ /^de_lin_dist_penalty=(.*)$/)
            #{
            #    push(@lambdas, $1);
            #}
        }
