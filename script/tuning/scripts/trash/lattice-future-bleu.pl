...
if(@ARGV<4) {
    print STDERR "\nusage: lattice-future-bleu.pl <lattice-file> <O-score-file> <src-file> <ref-files>\n\n";
    exit(-1);
}
...
#	next if($sent_id<1);
#	last if($sent_id>1);
#	next if($sent_id<1305);
#	last if($sent_id>1305);
#	next if($sent_id<191);
#	last if($sent_id>191);
...
#			print STDERR "cost log_100=$cost\n";
	...
	my %node_outgoremain;
	foreach my $node (keys %nodes) {
	    if(exists($edge_trans{$node})) {
		my $num=(keys %{ $edge_trans{$node} });
		$node_outgoremain{$node}=$num;
	    } else {
		$node_outgoremain{$node}=0;
	    }
	}
	...
	my %node_bleu;
	my %node_max;
	my %node_max_modelprob;
	my %node_maxarg;
	my %node_max_modelprob_arg;
	if($add_s_tag) {
	    @{ $node_hist{0} }=( '<s>' );
	} else {
	    @{ $node_hist{0} }=();
	...
	for(my $i=0; $i<@{ $node_O{0} }; $i++) {
#	    $node_O{0}[$i]/=5;
#	    $node_O{0}[$i]=1;
	}


#	print STDERR $base**&compute_O_bleu_vec(\@{ $node_O{0} }), "\n";
#	die;
	$node_max{0}=0;
	$node_max_modelprob{0}=0;
	if($node_O{0}[-1]==0) {
	    $node_bleu{0}=0;
	} else {
...
#			print STDERR "node_hist{$node_to}=$hist_string\n";
			...
			@{ $edge_O{$node_from}{$node_to} }=@{ $node_O{$node_from} };
...
#			&vector_addition(\@{ $node_O{$node_from} },\@edge_counts,\@{ $edge_O{$node_from}{$node_to} });
			my $combined_counts=&vector_addition(\@{ $node_O{$node_from} },\@edge_counts);

			
			my $bleu_to=&compute_O_bleu($sent_id,$edge_srclength{$node_from}{$node_to},\@{ $node_O{$node_from} },\@edge_counts,\@counts_to);
			
#			@{ $edge_O{$node_from}{$node_to} }=@counts_to;
			@{ $edge_O{$node_from}{$node_to} }=@$combined_counts;
...
#			print  "edge_bleu{$node_from}{$node_to}=$edge_bleu{$node_from}{$node_to}\n";
			...
			foreach my $node_from (keys %{ $edge_inv{$node_to} }) {
			    ...
			    if(!defined($node_max_modelprob{$node_to}) 
			       || $node_max_modelprob{$node_from}+$edge_modelcost{$node_from}{$node_to}>$node_max_modelprob{$node_to}) {
				$node_max_modelprob{$node_to}=$node_max_modelprob{$node_from}+$edge_modelcost{$node_from}{$node_to};
				$node_max_modelprob_arg{$node_to}=$node_from;
			    }
			}
...
#	die;
	...
	my %node_max_outgoing_O;
	my %node_max_outgoing_bleu;
	my %node_max_incoming_O;
	my %node_max_incoming_bleu;
	my %edge_max_future_bleu;

	my %edge_max_future_modelprob;
	my %node_max_outgoing_modelprob;

	my %visited_future_bleu;
#	@queue=( $end_node );
#	$node_max_future_bleu{$end_node}=$node_bleu{$end_node};
	foreach my $node_from (keys %{ $edge_inv{$end_node} }) {
	    my $bleu=&compute_bleu_vec(\@{ $edge_O{$node_from}{$end_node} },$sent_id);
	    $edge_max_future_bleu{$node_from}{$end_node}=$bleu;
	    if(!defined($node_max_outgoing_bleu{$end_node}) || $bleu>$node_max_outgoing_bleu{$end_node}) {
		$node_max_outgoing_bleu{$end_node}=$bleu;
		@{ $node_max_outgoing_O{$end_node} }=@{ $edge_O{$node_from}{$end_node} };
		$node_max_incoming_bleu{$end_node}=$bleu;
		@{ $node_max_incoming_O{$end_node} }=@{ $edge_O{$node_from}{$end_node} };
	    }
	    $node_max_outgoing_bleu{$node_from}=$bleu;
	    @{ $node_max_outgoing_O{$node_from} }=@{ $edge_O{$node_from}{$end_node} };

	    my $prob_modelcost=$node_max_modelprob{$end_node};
	    $node_max_outgoing_modelprob{$node_from}=$prob_modelcost;
	    $edge_max_future_modelprob{$node_from}{$end_node}=$prob_modelcost;
	...
	while(@queue>0) {
		...
		next if($node_to==0);
		foreach my $node_from (keys %{ $edge_inv{$node_to} }) {
		    my $bleu=&compute_bleu_vec(\@{ $edge_O{$node_from}{$node_to} },$sent_id);
		    if(!defined($node_max_incoming_bleu{$node_to}) || $bleu>$node_max_incoming_bleu{$node_to}) {
			$node_max_incoming_bleu{$node_to}=$bleu;
			@{ $node_max_incoming_O{$node_to} }=@{ $edge_O{$node_from}{$node_to} };
		    }
		}

#		if($node_to==3) {
#		    my $bleu=&compute_bleu_vec(\@{ $node_max_outgoing_O{$node_to} },$sent_id);
#		}

		foreach my $node_from (keys %{ $edge_inv{$node_to} }) {
		    my $edge_O_adjusted=&vector_addition(\@{ $edge_O{$node_from}{$node_to} },
							 &vector_subtraction(\@{ $node_max_outgoing_O{$node_to} },\@{ $node_max_incoming_O{$node_to} }));
		    
		    $edge_max_future_bleu{$node_from}{$node_to}=&compute_bleu_vec($edge_O_adjusted,$sent_id);

#NEXT:		    
		    $edge_max_future_modelprob{$node_from}{$node_to}
		    =(($node_max_outgoing_modelprob{$node_to}
		      -($node_max_modelprob{$node_max_modelprob_arg{$node_to}}+$edge_modelcost{$node_max_modelprob_arg{$node_to}}{$node_to}))
		      +$node_max_modelprob{$node_from}+$edge_modelcost{$node_from}{$node_to});

		    if(!defined($node_max_outgoing_bleu{$node_from})
		       || $edge_max_future_bleu{$node_from}{$node_to}>$node_max_outgoing_bleu{$node_from}) {
			$node_max_outgoing_bleu{$node_from}=$edge_max_future_bleu{$node_from}{$node_to};
			@{ $node_max_outgoing_O{$node_from} }=@$edge_O_adjusted;
		    }
		    if(!defined($node_max_outgoing_modelprob{$node_from})
		       || $edge_max_future_modelprob{$node_from}{$node_to}>$node_max_outgoing_modelprob{$node_from}) {
			$node_max_outgoing_modelprob{$node_from}=$edge_max_future_modelprob{$node_from}{$node_to};
		    }
		    
		    
		    $node_outgoremain{$node_from}--;
		    if($node_outgoremain{$node_from}==0) {
			$queue_next{$node_from}=1;
		    }
		}
		$visited_future_bleu{$node_to}=1;
	    }
	    undef @queue;
	    foreach my $node_from (sort {$a<=>$b} (keys %queue_next)) {
		if(!exists($visited_future_bleu{$node_from})) {
		    push(@queue,$node_from);
		}
	    }
	...
	my %node_max_future_bleu;
	my %node_max_future_O;
	...
	while(@queue>0) {
		...
		next if($node_to==0);
		foreach my $node_from (keys %{ $edge_inv{$node_to} }) {
#		    print STDERR "node_O(to){$node_to}=", join(' ',@{ $node_O{$node_to} }), "\n";
#		    print STDERR "edge_O{$node_from}{$node_to}=", join(' ',@{ $edge_O{$node_from}{$node_to} }), "\n";
#		    print STDERR "node_O(from){$node_from}=", join(' ',@{ $node_O{$node_from} }), "\n\n";

		    if($node_to==3) {
			print STDERR "node_max_future_O{$node_to}=", join(' ',@{ $node_max_future_O{$node_to} }), "\n";
			print STDERR "node_O(to){$node_to}=", join(' ',@{ $node_O{$node_to} }), "\n";
			print STDERR "edge_O{$node_from}{$node_to}=", join(' ',@{ $edge_O{$node_from}{$node_to} }), "\n";
			print STDERR "edge_O{$node_to}{798}=", join(' ',@{ $edge_O{$node_to}{798} }), "\n";
			my $vec;
			$vec=&vector_addition(\@{ $edge_O{$node_from}{$node_to} },&vector_subtraction(\@{ $node_max_future_O{$node_to} },\@{ $node_O{$node_to} }));
			print STDERR "vec=", join(' ',@$vec), "\n";
			print STDERR "edge_max_future_bleu{$node_to}{798}=", exp($edge_max_future_bleu{$node_to}{798}), "\n";
			print STDERR "re-edge_max_future_bleu{$node_to}{798}=", exp(&compute_bleu_vec(\@{ $node_max_future_O{$node_to} },$sent_id)), "\n";

		    }

		    $edge_max_future_bleu{$node_from}{$node_to}=
#			&compute_bleu_vec(&vector_addition(\@{ $node_O{$node_from} },&vector_subtraction(\@{ $node_max_future_O{$node_to} },\@{ $edge_O{$node_from}{$node_to} })),$sent_id);
			&compute_bleu_vec(&vector_addition(\@{ $edge_O{$node_from}{$node_to} },&vector_subtraction(\@{ $node_max_future_O{$node_to} },\@{ $node_O{$node_to} })),$sent_id);
#		    if($node_to==3) {
#			print STDERR "edge_max_future_bleu{$node_from}{$node_to}=", exp($edge_max_future_bleu{$node_from}{$node_to}), "\n";
#		    }
		    if(!defined($node_max_future_bleu{$node_from})
		       || $edge_max_future_bleu{$node_from}{$node_to}>$node_max_future_bleu{$node_from}) {
			$node_max_future_bleu{$node_from}=$edge_max_future_bleu{$node_from}{$node_to};
			@{ $node_max_future_O{$node_from} }=@{ $edge_O{$node_from}{$node_to} };
			if($node_from==3) {
			    print STDERR "update:node_max_future_bleu{$node_from}=", exp($node_max_future_bleu{$node_from}), "\n";
			    print STDERR "edge_max_future_bleu{$node_from}{$node_to}=", exp($edge_max_future_bleu{$node_from}{$node_to}), "\n";
			}
		    }
		    $node_outgoremain{$node_from}--;
...
#		    my $cost=exp($edge_max_future_bleu{$node_from}{$node_to});
		    my $cost=sprintf("%.4f",exp($edge_max_future_bleu{$node_from}{$node_to}));
		    $cost.=":". sprintf("%.6f",$edge_max_future_modelprob{$node_from}{$node_to});
		    $cost.=":". sprintf("%.6f",$edge_modelcost{$node_from}{$node_to});
		    my $edge_O_string=join(',',@{ $edge_O{$node_from}{$node_to} });

#		    push(@from_parts,"$node_from\|\|\|$edge_transoutput{$node_from}{$node_to}:$edge_O_string\|\|\|$cost");
...
#		my $bleuvalue=exp($node_bleu{$node_to});
#		print "$node_to:$bleuvalue:$O_string\t", join(' ',@from_parts), "\n";
#		print "$node_to:$node_max_future_bleu{$node_to}\t", join(' ',@from_parts), "\n";
...
sub compute_bleu_vec {
    my($O_scores,$sent_id)=@_;

    my $ref_length=&compute_ref_seg_length($bp_strategy,$O_scores->[1],\%{ $ref_lengths[$sent_id] });
    ...
    my $bp=&compute_brevity_penalty($O_scores->[1],$ref_length);
...
sub compute_O_bleu_vec {
    ...
    my $bp=&compute_brevity_penalty($O_scores->[1],$O_scores->[-2]);
#    my $bleu=$O_scores->[-1]*$bp*exp($prec);
    my $bleu=$bp*exp($prec);
    return &log_nulldef($bleu);
#    return $bleu;
...
#	$prec+= (1/$max_n) * 1;
    ...
    my $bp=&compute_brevity_penalty($O_comb->[1],$O_comb->[-2]);
...
sub vector_addition {
    ...
    for(my $i=0; $i<@$vec1 && $i<@$vec2; $i++) {
	$vec_add[$i]=$vec1->[$i]+$vec2->[$i];
    }
    return \@vec_add;
}

sub vector_subtraction {
    my($vec1,$vec2)=@_;
    my @vec_sub;
