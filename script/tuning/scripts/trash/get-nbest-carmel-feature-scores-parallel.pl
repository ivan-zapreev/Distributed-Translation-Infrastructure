...
for(my $i=0; $i<@batches; $i++) {
    ...
    my $call="nohup sh -c \'$OISTERHOME/tuning/scripts/get-nbest-carmel-feature-scores.NEW.pl $lattice_file:$from\-$to $feature_scores_file $feature_id2name_file $nbest $external_path >& $lattice_file.err.$i.log\; touch $finished_file\' \&";
    print STDERR "$call\n";
    system($call);
    $num_active_jobs++;
