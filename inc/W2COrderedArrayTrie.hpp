        ...
        namespace tries {
            ...
            protected:
                ...
                virtual TSubArrReference & make_M_N_GramEntry(const TModelLevel & level, const TShortId & wordId) {
                    ...
                    TSubArrReference & ref = m_MN_gram_word_2_data[mgram_idx][wordId];
                    ...
                    if (DO_SANITY_CHECKS && (ref.endIdx >= m_MN_gram_num_ctx_ids[mgram_idx])) {
                        ...
                        msg << "The maximum allowed number of " << SSTR(level)
                                << "-grams: " << SSTR(m_MN_gram_num_ctx_ids[mgram_idx])
                                << " is exceeded )!";
                        throw Exception(msg.str());
                    }
                ...
                virtual const TSubArrReference& get_M_N_GramEntry(const TModelLevel & level, const TShortId & wordId) {
                    ...
                    //First get the sub-array reference. 
                    const TSubArrReference & ref = m_MN_gram_word_2_data[mgram_idx][wordId];
                ...
                virtual const TProbBackOffEntryPair& get_M_GramDataRef(const TModelLevel level, const TShortId wordId, const TLongId ctxId) {
                    ...
                    //First get the sub-array reference. 
                    const TSubArrReference & ref = get_M_N_GramEntry(level, wordId);

                    //Check if there are elements for this context
                    TShortId found_idx;
                    if (binarySearch<TCtxIdProbBackOffEntryPair, TShortId, TShortId>(m_M_gram_data[mgram_idx], ref.beginIdx, ref.endIdx, ctxId, found_idx)) {
                        return m_M_gram_data[mgram_idx][found_idx].data;
                    } else {
                        LOG_DEBUG1 << "Unable to find " << SSTR(level) << "-gram data for wordId: " << SSTR(wordId)
                                << ", ctxId: " << SSTR(ctxId) << ", ctxId range: ["
                ...
                virtual const TLogProbBackOff& get_N_GramDataRef(const TShortId wordId, const TLongId ctxId) {
                    ...
                    //First get the sub-array reference. 
                    const TSubArrReference & ref = get_M_N_GramEntry(N, wordId);

                    //Search for the index using binary search
                    TShortId found_idx;
                    if (binarySearch<TCtxIdProbEntryPair, TShortId, TLongId>(m_N_gram_data, ref.beginIdx, ref.endIdx, ctxId, found_idx)) {
                        //return the reference to the probability
                        return m_N_gram_data[found_idx].prob;
                    } else {
                        LOG_DEBUG1 << "Unable to find " << SSTR(N) << "-gram data for wordId: " << SSTR(wordId)
                                << ", ctxId: " << SSTR(ctxId) << ", ctxId range: ["
                                << SSTR(m_N_gram_data[ref.beginIdx].ctxId)
                                << ", " << SSTR(m_N_gram_data[ref.endIdx].ctxId) << "]" << END_LOG;
                        throw out_of_range("not found");
                    }
                };
            ...
            private:
                ...
                TSubArrReference * m_MN_gram_word_2_data[NUM_M_N_GRAM_LEVELS];
                ...
                inline TLongId getContextId(TShortId wordId, TLongId ctxId, const TModelLevel level) {
                    ...
                    //First get the sub-array reference. 
                    TSubArrReference & ref = m_MN_gram_word_2_data[mgram_idx][ctxId];
                    ...
                    if (ref.beginIdx != UNDEFINED_ARR_IDX) {
                        ...
                        if (binarySearch<TCtxIdProbBackOffEntryPair, TShortId, TShortId>(m_M_gram_data[mgram_idx], ref.beginIdx, ref.endIdx, wordId, result)) {
                            return result;
                        } else {
            ...
            typedef W2COrderedArrayTrie<MAX_NGRAM_LEVEL> TFiveW2COrderedArrayTrie;
        }
    }
}
