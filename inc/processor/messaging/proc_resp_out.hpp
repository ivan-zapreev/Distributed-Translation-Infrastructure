        ...
        namespace bpbd {
            namespace server {
                namespace messaging {
                    ...
                    /**
                     * This class represents a processor job response message to be sent to the client
                     */
                    ...
                    public:
                        ...
                        /**
                         * Allows to set the text piece
                         * @param text_piece the text piece
                         * @param text_piece_idx the text piece index
                         * @param num_text_pieces the number of text pieces
                         */
                        inline void set_text(const string & text_piece, const uint64_t text_piece_idx, const uint64_t num_text_pieces) {
                            m_writer.String(TEXT_PIECE_IDX_FIELD_NAME);
                            m_writer.String(text_piece.c_str());
                            m_writer.String(TEXT_PIECE_FIELD_NAME);
                            m_writer.Int(text_piece_idx);
                            m_writer.String(NUM_TEXT_PIECES_FIELD_NAME);
                            m_writer.Int(num_text_pieces);
                        }
                    ...
                    private:
                        ...
                        proc_resp_out(const msg_type type, const job_id_type job_id,
                                const status_code code, const string & msg)
                        : outgoing_msg(msg_type::MESSAGE_TRANS_JOB_RESP), proc_resp() {
